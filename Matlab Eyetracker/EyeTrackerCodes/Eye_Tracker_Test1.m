...
for i = 1:1:lengthexp
    I = snapshot(cam);
    % image(img);
    % use 'dark' because both the iris as well as the pupil are darker than
    % the sclera
...
hold off;
    ...
    imshow(I)
    % create the vectors "centers" named "c" and "radii" named "ro"
    h = viscircles(ro,c);
    % below would show the placeholder with green circles
    % hRef = viscircles(centersBright, radiiBright,'Color','g');
    disp(ro(1))
% disconnect camera from the system
clear cam